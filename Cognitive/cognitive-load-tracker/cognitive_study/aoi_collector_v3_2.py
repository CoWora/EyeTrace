...
# ========== 确保可以导入项目根目录下的 Model 包 ==========
# 当前文件路径：EyeTrace/Cognitive/cognitive-load-tracker/cognitive_study/aoi_collector_v3_2.py
# 项目根目录为其向上第 3 级目录：EyeTrace
...
class TaskInfo:
    ...
    def duration(self) -> float:
        """与导出 tasks.csv 的列名一致，由起止时间计算（非独立存储字段）。"""
...
class CognitiveLoadFeedbackWindow:
    """
    认知负荷实时反馈窗口（Tkinter），与 OpenCV 采集窗口并行运行。
    每次任务结束后自动读取当前 task 的数据做预测并刷新展示。

    Tkinter 必须在独立线程中运行，这里用 threading 实现。
    """

    @classmethod
    def start(cls, predictor, collector, project_root: Path):
        """
        工厂方法：在独立线程中创建并运行窗口，返回窗口实例。
        窗口启动完成后才返回。
        """
        ...
        window._pending_task_ids: set = set()
        window._ready_event = threading.Event()
        window._shutdown_queue: queue.Queue = queue.Queue()
        window._result_queue: queue.Queue = queue.Queue()
        window._running = False
        window._thread = None

        def thread_target():
            window._root = tk.Tk()
            window._root.title("EyeTrace 认知负荷反馈")
            window._root.geometry("420x320")
            window._root.resizable(False, False)
            window._root.protocol("WM_DELETE_WINDOW", window._on_close)
            window._status_var = tk.StringVar(value="等待任务开始...")
            # 必须在 _schedule_refresh 之前为 True，否则第一次 after 永远不会挂上
            window._running = True
            window._create_widgets()
            window._schedule_refresh()
            window._ready_event.set()
            window._root.mainloop()
            window._running = False

        window._thread = threading.Thread(target=thread_target, daemon=True)
        window._thread.start()
        window._ready_event.wait()
        _info("[Feedback] 认知负荷反馈窗口已启动")
        return window

    def request_shutdown(self):
        """由采集主线程调用：请求在 Tk 线程内安全退出，避免 Tcl_AsyncDelete。"""
        try:
            self._shutdown_queue.put_nowait("quit")
        except Exception:
            pass
    ...
    def _create_widgets(self):
        ...
        columns = ("task", "level", "label")
        self._tree = ttk.Treeview(frame, columns=columns, show="headings", height=8)

        self._tree.heading("task", text="任务")
        self._tree.heading("level", text="等级")
        self._tree.heading("label", text="负荷类型")

        self._tree.column("task", width=90, anchor=tk.CENTER)
        self._tree.column("level", width=60, anchor=tk.CENTER)
        self._tree.column("label", width=240, anchor=tk.W)
        ...
        # 当前任务结果显示区
        self._current_frame = tk.Frame(self._root, bg="#34495E", bd=1, relief="solid")
        self._current_frame.pack(fill=tk.X, padx=12, pady=(0, 12))

        self._current_task_var = tk.StringVar(value="当前任务: 无")
        self._current_level_var = tk.StringVar(value="等待中...")

        tk.Label(
            self._current_frame, textvariable=self._current_task_var,
            font=("Microsoft YaHei", 9),
            bg="#34495E", fg="#ECF0F1"
        ).pack(anchor="w", padx=12, pady=(8, 2))

        self._current_level_label = tk.Label(
            self._current_frame, textvariable=self._current_level_var,
            font=("Microsoft YaHei", 22, "bold"),
            bg="#34495E", fg="#2ECC71"
        )
        self._current_level_label.pack(padx=12, pady=(0, 8))
    ...
    def _schedule_refresh(self):
        if not self._running:
            return
        self._root.after(500, self._do_refresh)

    def _do_refresh(self):
        # 主线程通过 queue 请求退出，必须在 Tk 线程内 destroy，避免 Tcl_AsyncDelete
        try:
            while True:
                cmd = self._shutdown_queue.get_nowait()
                if cmd == "quit":
                    self._running = False
                    try:
                        self._root.quit()
        ...
        # 预测线程只往队列塞结果，由本方法统一更新 Tk（避免非 Tk 线程调用 after）
        try:
            while True:
                item = self._result_queue.get_nowait()
                if item[0] == "ok":
                    _, task_id, level, label = item
                    self._add_result_row(task_id, level, label)
                    self._status_var.set(f"任务 {task_id} 预测完成 ✓")
                    color = _LOAD_COLORS.get(level, "#95A5A6")
                    self._current_level_label.config(text=f"L{level} {label}", fg=color)
                elif item[0] == "err":
                    self._status_var.set(item[1][:80] if len(item) > 1 else "预测出错")
        except queue.Empty:
            pass

        try:
            # 刷新当前任务状态（跨线程读取，无锁也可接受）
            current_task = self.collector.current_task
            if current_task:
                self._current_task_var.set(f"当前任务: {current_task.task_id} ({current_task.difficulty})")
                elapsed = time.time() - current_task.start_time
                self._current_level_var.set(f"进行中... {elapsed:.0f}s")
            else:
                self._current_task_var.set("当前任务: 无")
                self._current_level_var.set("等待中...")

            # 处理待预测任务：在独立线程中执行，避免冻结 UI
            if self._pending_task_ids and self.predictor is not None:
                task_to_predict = sorted(self._pending_task_ids)[0]
                self._pending_task_ids.discard(task_to_predict)
                threading.Thread(
                    target=self._predict_in_thread,
                    args=(task_to_predict,),
                    daemon=True
                ).start()
    ...
    def _predict_in_thread(self, task_id: str):
        """在独立线程中执行预测；结果写入 _result_queue，由 _do_refresh 在 Tk 线程更新 UI。"""
        try:
            temp_dir = self.project_root / "data" / "_temp_predict"
            os.makedirs(temp_dir, exist_ok=True)
            self._export_task_data(temp_dir, task_id)
            results = self.predictor.predict(str(temp_dir))
            if not results:
                self._result_queue.put(("err", f"任务 {task_id}: 特征提取失败"))
                return
            result = results[0]
            self._result_queue.put(("ok", task_id, result.relative_load_level, result.relative_load_label))
        except Exception as e:
            _warn(f"[Feedback] 预测失败: {e}")
            self._result_queue.put(("err", f"预测出错: {str(e)[:60]}"))

    def _add_result_row(self, task_id: str, level: int, label: str):
        """在列表中添加一行结果"""
        ...
        self._tree.tag_configure(tag_name, background=color, foreground="white")
        self._tree.insert("", 0, values=(task_id, f"L{level}", label), tags=(tag_name,))

    def _export_task_data(self, temp_dir: Path, task_id: str):
        """将当前 session 中指定 task 的数据导出为 CSV"""
            ...
            with open(path, 'w', newline='', encoding='utf-8') as f:
                ...
                for r in rows:
                    if isinstance(r, dict):
                        w.writerow([r.get(f, "") for f in headers])
                    else:
                        w.writerow([getattr(r, f) for f in headers])
            return path
        ...
        gaze_rows = [r for r in self.collector.session_data.gaze_records if r.task_id == task_id]
        _write_csv("gaze_data.csv", gaze_rows,
                   ['timestamp', 'gaze_x', 'gaze_y', 'screen_x', 'screen_y',
                    'yaw', 'pitch', 'aoi_region', 'aoi_name', 'is_luogu',
                    'is_fixation', 'fixation_id', 'task_id'])

        fix_rows = [r for r in self.collector.session_data.fixations if r.task_id == task_id]
        _write_csv("fixations.csv", fix_rows,
                   ['fixation_id', 'start_time', 'end_time', 'duration',
                    'center_x', 'center_y', 'aoi_region', 'task_id'])

        blink_rows = [r for r in self.collector.session_data.blinks if r.task_id == task_id]
        _write_csv("blinks.csv", blink_rows, ['timestamp', 'blink_id', 'ear', 'task_id'])

        trans_rows = [r for r in self.collector.session_data.transitions if r.task_id == task_id]
        _write_csv("aoi_transitions.csv", trans_rows, ['timestamp', 'from_aoi', 'to_aoi', 'task_id'])

        task_rows = [r for r in self.collector.session_data.tasks if r.task_id == task_id]
        _write_csv("tasks.csv", task_rows,
                   ['task_id', 'problem_id', 'difficulty', 'start_time', 'end_time', 'duration',
                    'result', 'subjective_difficulty', 'subjective_effort'])

        event_rows = [e for e in self.collector.session_data.events if e.get('task_id') == task_id]
        _write_csv("events.csv", event_rows, ['timestamp', 'type', 'task_id', 'description'])

        meta = {
            "session_id": self.collector.session_id,
            "start_time": self.collector.session_data.start_time,
            "end_time": time.time(),
            "total_gaze_records": len(gaze_rows),
            "total_fixations": len(fix_rows),
            "total_transitions": len(trans_rows),
            "total_blinks": len(blink_rows),
            "total_tasks": len(task_rows),
        }
        _write_json("session_meta.json", meta)
...
class CognitiveLoadCollector:
    def __init__(self, output_dir=None):
        """
        采集器：负责把本次 Session 的所有数据写入磁盘。

        output_dir:
            - 若为 None，则默认写入项目根目录下的 data/ 目录，即：
              C:\\Users\\YNS\\Desktop\\EyeTrace\\data
            - 若传入自定义路径，则写入该路径。
        """
        if output_dir is None:
            # PROJECT_ROOT 在文件开头已根据 __file__ 计算好（指向 EyeTrace 根目录）
        ...
        self.recording = True
        # 任务结束回调：feedback_window 等外部组件可注册
        self._task_end_callback: callable = None
    ...
    def end_task(self, result: str, screen_w: int, screen_h: int):
        ...
        # 触发认知负荷预测回调
        if self._task_end_callback is not None:
            try:
                self._task_end_callback(completed_task.task_id)
            except Exception:
                pass
...
def export_session_snapshot_for_realtime(collector, snapshot_dir: str) -> str:
    """
    为实时预测导出一次“快照”数据，不改变采集状态。

    与 export_data 类似，但不会调用 finalize_current_task，也不会修改 session_id。
    """
    os.makedirs(snapshot_dir, exist_ok=True)

    # 视线数据
    with open(os.path.join(snapshot_dir, "gaze_data.csv"), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['timestamp', 'gaze_x', 'gaze_y', 'screen_x', 'screen_y', 'yaw', 'pitch',
                   'aoi_region', 'aoi_name', 'is_luogu', 'is_fixation', 'fixation_id', 'task_id'])
        for r in collector.session_data.gaze_records:
            w.writerow([r.timestamp, r.gaze_x, r.gaze_y, r.screen_x, r.screen_y, r.yaw, r.pitch,
                       r.aoi_region, r.aoi_name, r.is_luogu, r.is_fixation, r.fixation_id, r.task_id])

    # 注视
    with open(os.path.join(snapshot_dir, "fixations.csv"), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['fixation_id', 'start_time', 'end_time', 'duration', 'center_x', 'center_y', 'aoi_region', 'task_id'])
        for r in collector.session_data.fixations:
            w.writerow([r.fixation_id, r.start_time, r.end_time, r.duration, r.center_x, r.center_y, r.aoi_region, r.task_id])

    # AOI 转换
    with open(os.path.join(snapshot_dir, "aoi_transitions.csv"), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['timestamp', 'from_aoi', 'to_aoi', 'task_id'])
        for r in collector.session_data.transitions:
            w.writerow([r.timestamp, r.from_aoi, r.to_aoi, r.task_id])

    # 任务
    with open(os.path.join(snapshot_dir, "tasks.csv"), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['task_id', 'problem_id', 'difficulty', 'start_time', 'end_time', 'duration',
                   'result', 'subjective_difficulty', 'subjective_effort'])
        for t in collector.session_data.tasks:
            duration = t.end_time - t.start_time if t.end_time and t.start_time else 0
            w.writerow([t.task_id, t.problem_id, t.difficulty, t.start_time, t.end_time, duration,
                       t.result, t.subjective_difficulty, t.subjective_effort])

    # 事件
    with open(os.path.join(snapshot_dir, "events.csv"), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['timestamp', 'type', 'task_id', 'description'])
        for e in collector.session_data.events:
            w.writerow([e['timestamp'], e['type'], e['task_id'], e['description']])

    # 眨眼
    with open(os.path.join(snapshot_dir, "blinks.csv"), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['timestamp', 'blink_id', 'ear', 'task_id'])
        for b in collector.session_data.blinks:
            w.writerow([b.timestamp, b.blink_id, b.ear, b.task_id])

    # meta（end_time 使用当前时间，表示“快照时刻”）
    meta = {
        "session_id": collector.session_id,
        "start_time": collector.session_data.start_time,
        "end_time": time.time(),
        "total_gaze_records": len(collector.session_data.gaze_records),
        "total_fixations": len(collector.session_data.fixations),
        "total_transitions": len(collector.session_data.transitions),
        "total_blinks": len(collector.session_data.blinks),
        "total_tasks": len(collector.session_data.tasks),
        "snapshot": True,
    }
    with open(os.path.join(snapshot_dir, "session_meta.json"), 'w', encoding='utf-8') as f:
        json.dump(meta, f, indent=2)
...
def main():
    ...
    # 加载模型（同时兼容从不同工作目录启动）
    # 你现在可能把权重放在以下任一路径（都支持）：
    # - Cognitive\\models\\L2CSNet_gaze360.pkl
    # - Cognitive\\cognitive-load-tracker\\models\\L2CSNet_gaze360.pkl
    # - （兼容旧用法）当前工作目录下的 models\\L2CSNet_gaze360.pkl
    weights_name = "L2CSNet_gaze360.pkl"
    script_dir = Path(__file__).resolve().parent           # ...\\Cognitive\\cognitive-load-tracker\\cognitive_study
    clt_root = script_dir.parent                           # ...\\Cognitive\\cognitive-load-tracker
    cognitive_root = clt_root.parent                       # ...\\Cognitive
    cwd = Path.cwd()

    candidates = [
        # 优先考虑你现在实际使用的两个目录
        clt_root / "models" / weights_name,                # Cognitive\\cognitive-load-tracker\\models
        cognitive_root / "models" / weights_name,          # Cognitive\\models
        # 再兼容从项目根目录等位置启动的旧用法
        cwd / "models" / weights_name,
        cwd / "Cognitive" / "models" / weights_name,
        cwd / "Cognitive" / "cognitive-load-tracker" / "models" / weights_name,
    ]
    ...
    # ========== 实时认知负荷预测器（可选） ==========
    # 默认每 5 分钟对“当前已采集的数据”导出一次快照，并用 ET_model 做一次粗略预测。
    # 若模型或依赖缺失，将自动降级为“仅采集”，不影响正常使用。
    # 拓展：对“当前任务”维护两类指标：
    #   - 从任务开始以来所有预测结果的平均等级（总认知负荷）
    #   - 最近 5 分钟内的平均等级（短期认知负荷）
    realtime_interval = 300.0  # 秒
    realtime_last_run = time.time()
    realtime_status_text = ""
    # 历史记录：[(timestamp, task_id, relative_load_level)]
    realtime_history = []
    # 当前任务的聚合负荷（平均等级），None 表示暂不可用
    realtime_total_avg = None
    realtime_recent_5m_avg = None
    realtime_predictor = None

    try:
        from Model.ET_model.predict_utils import SessionPredictor

        _info("实时预测模块已启用：任务结束后将自动预测认知负荷等级")
        # task 级 SVM 与 task 级 PCA/特征表必须同粒度（outputs 下多为 session 级，列数与模型不一致会报 feature names 错误）
        _model_root = PROJECT_ROOT / "Model" / "ET_model"
        svm_model = _model_root / "outputs_supervised_task" / "model_svm.joblib"
        pca_model = _model_root / "outputs_task_cluster" / "pca_model.joblib"
        feat_template = _model_root / "outputs_task_cluster" / "features.csv"

        realtime_predictor = SessionPredictor(
            classifier_model=str(svm_model),
            pca_model=str(pca_model),
            features_template=str(feat_template),
        )
        _info(f"  - SVM 模型: {svm_model} ({'存在' if svm_model.exists() else '不存在'})")
        _info(f"  - PCA 模型: {pca_model} ({'存在' if pca_model.exists() else '不存在'})")
        _info(f"  - 特征模板: {feat_template} ({'存在' if feat_template.exists() else '不存在'})")
    except Exception as e:
        _warn(f"实时预测模块初始化失败，将仅采集数据: {e}")
        realtime_predictor = None

    # 创建认知负荷反馈窗口（在预测器可用时）
    feedback_window = None
    if realtime_predictor is not None:
        try:
            feedback_window = CognitiveLoadFeedbackWindow.start(realtime_predictor, collector, PROJECT_ROOT)
            collector._task_end_callback = feedback_window.notify_task_completed
        except Exception as e:
            _warn(f"反馈窗口创建失败: {e}")
    ...
    try:
        while True:
            ...
            # ========== 实时预测：按间隔导出快照并调用模型 ==========
            if (
                realtime_predictor is not None
                and (now - realtime_last_run) >= realtime_interval
                and len(collector.session_data.gaze_records) > 100  # 避免数据过少
            ):
                snapshot_dir = os.path.join(
                    collector.output_dir,
                    f"{collector.session_id}_realtime",
                )
                try:
                    export_session_snapshot_for_realtime(collector, snapshot_dir)
                    results = realtime_predictor.predict(snapshot_dir)
                    if results:
                        # 使用当前时间作为这些预测点的时间戳
                        ts_now = now
                        for r in results:
                            task_id = r.task_id or "none"
                            realtime_history.append((ts_now, task_id, r.relative_load_level))

                        # 仅针对“当前任务”计算两类聚合指标
                        if collector.current_task:
                            cur_tid = collector.current_task.task_id
                            # 该任务历史上所有预测点
                            all_points = [
                                lvl for (ts, tid, lvl) in realtime_history if tid == cur_tid
                            ]
                            # 最近 5 分钟内的预测点
                            cutoff = ts_now - 300.0
                            recent_points = [
                                lvl
                                for (ts, tid, lvl) in realtime_history
                                if tid == cur_tid and ts >= cutoff
                            ]

                            realtime_total_avg = (
                                sum(all_points) / len(all_points) if all_points else None
                            )
                            realtime_recent_5m_avg = (
                                sum(recent_points) / len(recent_points)
                                if recent_points
                                else None
                            )

                            # 文本状态优先展示当前任务的“总平均等级”
                            if realtime_total_avg is not None:
                                # 四舍五入到 1 位小数
                                realtime_status_text = (
                                    f"Task {cur_tid}: 总 L{realtime_total_avg:.1f} / "
                                    f"近5分钟 L{realtime_recent_5m_avg:.1f}"
                                    if realtime_recent_5m_avg is not None
                                    else f"Task {cur_tid}: 总 L{realtime_total_avg:.1f}"
                                )
                            else:
                                # 无法计算当前任务的历史点，退回到“最近一次样本”的等级
                                latest = results[-1]
                                realtime_status_text = (
                                    f"L{latest.relative_load_level} "
                                    f"{latest.relative_load_label}"
                                )
                        else:
                            # 没有正在进行的任务时，仍然给出最近一次样本的等级
                            latest = results[-1]
                            realtime_status_text = (
                                f"L{latest.relative_load_level} "
                                f"{latest.relative_load_label}"
                        ...
                        _info(
                            f"[Realtime] 快照预测完成，共 {len(results)} 个 task 样本；"
                            f"当前任务聚合负荷: "
                            f"总={realtime_total_avg if realtime_total_avg is not None else 'N/A'}, "
                            f"近5分钟={realtime_recent_5m_avg if realtime_recent_5m_avg is not None else 'N/A'}"
                        )
                    else:
                        realtime_status_text = "数据不足"
                        _warn("[Realtime] 当前数据不足，无法提取特征")
                except Exception as e:
                    realtime_status_text = "预测失败"
                    _warn(f"[Realtime] 实时预测失败: {e}")
                finally:
                    realtime_last_run = now
            ...
            # 实时认知负荷（若已计算）
            if realtime_status_text:
                # 第一行：文字状态（包含任务 ID 与两种窗口的平均等级）
                cv2.putText(
                    frame,
                    f"Realtime: {realtime_status_text}",
                    (w-320, 92),
                ...
                # 第二行：更结构化地单独列出两个指标，便于一眼区分
                if collector.current_task:
                    label_y = 108
                    if realtime_total_avg is not None:
                        cv2.putText(
                            frame,
                            f"Total since task start: L{realtime_total_avg:.1f}",
                            (w-320, label_y),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.35,
                            (200, 220, 255),
                    ...
                    if realtime_recent_5m_avg is not None:
                        cv2.putText(
                            frame,
                            f"Last 5 min: L{realtime_recent_5m_avg:.1f}",
                            (w-320, label_y),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.35,
                            (200, 255, 200),
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
        # 关闭认知负荷反馈窗口（禁止在主线程里直接 after/destroy Tk）
        if feedback_window is not None:
            try:
                feedback_window.request_shutdown()
                if getattr(feedback_window, "_thread", None) is not None:
                    feedback_window._thread.join(timeout=3.0)
            except Exception:
                pass
        cap.release()
